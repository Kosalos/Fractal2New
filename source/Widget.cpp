...
const char* WidgetData::valueString() {
...
const char* WidgetData::displayString(bool showV) {
...
void Widget::create(int id, const char* className, const char* title, HWND parent, HINSTANCE hInstance, int height) {
	WNDCLASSEX wcex;
...
void Widget::addLegend(const char* nlegend) {
	if (count >= MAX_WIDGETS) ABORT(-1);
...
void Widget::addBoolean(const char* nlegend, void* ptr) {
	if (count >= MAX_WIDGETS) ABORT(-1);
