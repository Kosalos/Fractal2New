...
struct WidgetData {
	...
	const char* displayString(bool showV);
};
...
public:
	...
	void create(int id, const char* className, const char *title,HWND parent, HINSTANCE inst, int height);
	void destroy();
	...
	void reset();
	void addLegend(const char* nlegend);
	void addBoolean(const char* nlegend, void* ptr);
